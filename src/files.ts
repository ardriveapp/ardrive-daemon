...
async function queueFile(filePath: string, login: string, driveId: string, drivePrivacy: string): Promise<any> {
	...
	if (extension !== '.enc' && stats.size !== 0 && !fileName.startsWith('~$')) {
		...
		const parentFolder: core.ArFSFileMetaData = await getDb.getFolderFromSyncTable(driveId, parentFolderPath);
		let parentFolderId = '';
		...
		const fileHash = await core.checksumFile(filePath);
		...
		const newFileToQueue: core.ArFSFileMetaData = {
			id: 0,
			login,
			appName,
			appVersion,
...
async function queueFolder(
	...
	if (isQueuedOrCompleted || fileName === 'New folder') {
		...
		const folderToQueue: core.ArFSFileMetaData = {
			id: 0,
			login,
			appName,
			appVersion,
...
export async function startWatchingFolders(user: core.ArDriveUser): Promise<any> {
	const drives: core.ArFSDriveMetaData[] = await getDb.getAllPersonalDrivesByLoginFromDriveTable(user.login);
	const stoppers: Array<() => Promise<void>> = [];
	if (drives !== undefined) {
		drives.forEach(async (drive: core.ArFSDriveMetaData) => {
			const rootFolder: core.ArFSFileMetaData = await getDb.getDriveRootFolderFromSyncTable(drive.rootFolderId);
