...
export function copyFolder(oldFolderPath: string, newFolderPath: string): string {
...
export async function checkForMissingLocalFiles(): Promise<string> {
	const localFiles: core.ArFSFileMetaData[] = await getDb.getAllLatestFileAndFolderVersionsFromSyncTable();
	await asyncForEach(localFiles, async (localFile: core.ArFSFileMetaData) => {
...
export async function backupWallet(backupWalletPath: string, wallet: core.Wallet, owner: string): Promise<string> {
	try {
...
export async function setAllFolderHashes(): Promise<string> {
	try {
		...
		const allFolders: core.ArFSFileMetaData[] = await getDb.getAllLocalFoldersFromSyncTable();
		// Update the hash of the parent folder
		await asyncForEach(allFolders, async (folder: core.ArFSFileMetaData) => {
...
export async function setAllFileHashes(): Promise<string> {
	try {
		const allFiles: core.ArFSFileMetaData[] = await getDb.getAllUnhashedLocalFilesFromSyncTable();
		// Update the hash of the file
		await asyncForEach(allFiles, async (file: core.ArFSFileMetaData) => {
			const fileHash = await core.checksumFile(file.filePath);
...
export async function setAllParentFolderIds(): Promise<string> {
	try {
		const allFilesOrFolders: core.ArFSFileMetaData[] = await getDb.getAllMissingParentFolderIdsFromSyncTable();
		await asyncForEach(allFilesOrFolders, async (fileOrFolder: core.ArFSFileMetaData) => {
			const parentFolderPath = dirname(fileOrFolder.filePath);
			const parentFolder: core.ArFSFileMetaData = await getDb.getFolderFromSyncTable(
...
export async function setFolderChildrenPaths(folder: core.ArFSFileMetaData): Promise<string> {
	const childFilesAndFolders: core.ArFSFileMetaData[] = await getDb.getFilesAndFoldersByParentFolderFromSyncTable(
		folder.fileId
	);
	if (childFilesAndFolders !== undefined) {
		await asyncForEach(childFilesAndFolders, async (fileOrFolder: core.ArFSFileMetaData) => {
...
export async function setNewFilePaths(): Promise<string> {
	const filesToFix: core.ArFSFileMetaData[] = await getDb.getAllMissingPathsFromSyncTable();
	await asyncForEach(filesToFix, async (fileToFix: core.ArFSFileMetaData) => {
...
export async function updateFilePath(file: core.ArFSFileMetaData): Promise<string> {
	try {
