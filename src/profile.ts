...
export async function setupDrives(login: string, syncFolderPath: string): Promise<string> {
	try {
		...
		// get all drives
		const drives: core.ArFSDriveMetaData[] = await getAllDrivesByLoginFromDriveTable(login);

		// for each drive, check if drive folder exists
		await asyncForEach(drives, async (drive: core.ArFSDriveMetaData) => {
			...
			const driveFolderEntity: core.ArFSFileMetaData = await getFolderFromSyncTable(drive.driveId, drivePath);
			if (driveFolderEntity === undefined) {
				...
				let isPublic = 1;
				let rootFolderMetaData: core.ArFSRootFolderMetaData = {
					metaDataTxId: '',
					cipher: '',
					cipherIV: ''
				};
				if (drive.drivePrivacy === 'private') {
					isPublic = 0;
					rootFolderMetaData = await core.getPrivateDriveRootFolderTxId(drive.driveId, drive.rootFolderId);
				} else {
					// Get the root folder ID for this drive
					rootFolderMetaData.metaDataTxId = await core.getPublicDriveRootFolderTxId(
						drive.driveId,
						drive.rootFolderId
					);
				}
				...
				const driveRootFolderToAdd: core.ArFSFileMetaData = {
					id: 0,
					login,
...
export async function addNewUser(loginPassword: string, user: core.ArDriveUser): Promise<string> {
	try {
		const encryptedWalletPrivateKey = await core.encryptText(user.walletPrivateKey, loginPassword);
		const encryptedDataProtectionKey = await core.encryptText(user.dataProtectionKey, loginPassword);
...
export async function updateUserSyncFolderPath(login: string, newSyncFolderPath: string): Promise<string> {
	try {
		...
		const filesToMove: core.ArFSFileMetaData[] = await getAllFilesByLoginFromSyncTable(login);
		await asyncForEach(filesToMove, async (fileToMove: core.ArFSFileMetaData) => {
...
export async function addSharedPublicDrive(user: core.ArDriveUser, driveId: string): Promise<string> {
	try {
		// Get the drive information from arweave using gql
		const sharedPublicDrive: core.ArFSDriveMetaData = await core.getSharedPublicDrive(driveId);
		...
		// Get the root folder ID for this drive
		const metaDataTxId = await core.getPublicDriveRootFolderTxId(
...
export async function deleteUserAndDrives(login: string): Promise<string> {
	...
	const drivesToDelete: core.ArFSDriveMetaData[] = await getAllDrivesByLoginFromDriveTable(login);
	// Delete drives and files matching login
	await asyncForEach(drivesToDelete, async (drive: core.ArFSDriveMetaData) => {
...
export async function passwordCheck(loginPassword: string, login: string): Promise<boolean> {
	try {
		const user: core.ArDriveUser = await getUserFromProfile(login);
		user.walletPrivateKey = await core.decryptText(JSON.parse(user.walletPrivateKey), loginPassword);
...
export async function getUser(loginPassword: string, login: string): Promise<core.ArDriveUser> {
	const user: core.ArDriveUser = await getUserFromProfile(login);
	user.dataProtectionKey = await core.decryptText(JSON.parse(user.dataProtectionKey), loginPassword);
	user.walletPrivateKey = await core.decryptText(JSON.parse(user.walletPrivateKey), loginPassword);
	console.log('');
